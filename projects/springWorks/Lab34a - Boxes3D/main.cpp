...
glBindTexture(GL_TEXTURE_2D,textures[3]);
displaytext( "work done on a spring ",800,900 );
        displaytext( "The defining character of a spring is that it resists ",800,850 );
        displaytext( "displacement from its rest position with a force which increases linearly ",800,800 );
//
//
//
                    //       2
   ...
   //
   //
displaytext( "restoring force  =  - k * (displacement) ",800,750 );
displaytext( "where k is called the spring constant. It has units of Newtons per meter.",800,700 );
displaytext( "When a spring pulls something, or pushes something, over a distance x, it does work ",800,650 );
displaytext( "work  =  1/2 * k * x^2",800,400 );
displaytext( "If a spring is compressed (or stretched) it stores energy equal to the work performed  ",800,550 );
           displaytext( " to compress (or stretch) it.We might call this spring potential energy. ",800,500 );
    glFlush();
...
int  main(int argc,char** argv)
{
    ...
    glutCreateWindow("work done on spring");
    setup();
    loadImages();
